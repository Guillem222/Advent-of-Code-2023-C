...
// Returns the prediction of the next number of a sequence
//it assumes that curr_seq is not all zeroes.
int predict_next_number(int* curr_seq, int size_curr_seq){
    int last_nums[MAX_DEPTH];
    int depth = 0;
    last_nums[depth++] = curr_seq[size_curr_seq - 1];
    ...
    while(1){
        ...
        last_nums[depth++] = below_seq[size_top_seq - 2];
    ...
    for(int i = (depth - 1); i >= 0;i--){
        last_pred =  last_nums[i] + last_pred;
    }
...
int main(){
    ...
    while ((read = getline(&line, &len, file)) != -1) {
        ...
        total_sum_pred += predict_next_number(curr_seq,size_curr_seq);
