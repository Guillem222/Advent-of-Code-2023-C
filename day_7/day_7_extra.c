...
int get_hand_type_no_joker(char* hand){
...
int exists_in_arr(char set_cards[5], char card){
    for(int i = 0; i < 5;i++){
        if(set_cards[i] == card) return 1;
    }
    return 0;
}

//Recursive function for all permutations with repetition.
int calc_all_perm(int current_depth, int num_of_jokers, char set_of_cards[5],int num_set_cards, char hand[5 + 1], char mod_hand[5 + 1], int pos_joker[5], int* best_hand){
    if(current_depth == (num_of_jokers - 1)){

        for(int i = 0; i < num_set_cards;i++){
            mod_hand[pos_joker[current_depth]] = set_of_cards[i];
            int hand_type = get_hand_type_no_joker(mod_hand);
...
int get_hand_type(char* hand){
    ...
    //Get all the different cards of the hand
    char set_cards[5];
    int num_set_cards = 0;
    memset(set_cards,'0',5);
    for(int i = 0; i < 5; i++){
        if((hand[i] != 'J') && !exists_in_arr(set_cards,hand[i])){
            set_cards[num_set_cards++] = hand[i];
        }
    ...
    //Calc all permutations with repetition
    
    int best_hand = -1;
    char mod_hand[5 + 1];
    strcpy(mod_hand,hand);

    calc_all_perm(0,num_of_jokers,set_cards,num_set_cards,hand,mod_hand,pos_joker,&best_hand);
